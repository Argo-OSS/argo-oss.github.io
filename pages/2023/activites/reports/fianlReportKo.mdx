...
Argo Workflows의 3.5버전 릴리즈 이후에도, 계속하여 컨트리뷰터들의 `DX개선`을 위한 CI및 테스트 환경, Nix, `Controller`, `Template` 스펙의 새로운 피쳐 개발,
Go Lang 버전 업데이트 등의 높은 난이도의 컨트리뷰션등을 이어나가고 있으며,
다음 릴리즈 배포에는 더욱 많은 기여로 Argo Workflows가 더욱 훌륭한 툴로 거듭나는데 의미있는 성과를 보태는 중입니다.
